...
function Booking() {
    ...
    const handleBooking = () => {
        ...
        switch (errorCode) {
            case 1:
                toast.error("The checkout date cannot be more than 30 days before the check-in date\r\n", {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
        ...
        if (!validateID(idCard)) {
            toast.error("ID Card must be 9 or 12 length and contains number only\r\n", {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/api/v1/customer/`, {
            cusName: cusName,
            cusIDcard: idCard,
            cusPhone: phone
        }).then(
            axios.get(`${process.env.REACT_APP_API_URL}/api/v1/customer/${idCard}`)
                .then(
                    (resultCustomer => (
                        axios.get(`${process.env.REACT_APP_API_URL}/api/v1/room/${roomTypeID}`)
                            .then(resultRoom => (
                                axios.post(`${process.env.REACT_APP_API_URL}/api/v1/book`, {
                ...
                <div class="container flex-column">
                    <form action="" class="form">
                        <div class="input input-group">
                            ...
                            <input onChange={(e) => setCusName(e.target.value)} type="text" class="input" id="name"></input>
                        </div>
                        <div class="input input-group">
                            <label for="phone" class="input-label">Telephone</label>
                            <input onChange={(e) => setPhone(e.target.value)} type="text" class="input" id="phone"></input>
                        </div>
                        <div class="input input-group">
                            <label for="check-out" class="input-label">ID card</label>
                            <input onChange={(e) => setIDCard(e.target.value)} type="text" class="input" id="check-out"></input>
                        </div>
                    </form>
                    <form action="" class="form">
                        <div class="input input-group w-25">
                            ...
                            )} type="date" class="input" id="check-in"></input>
                        </div>
                        <div class="input input-group w-25">
                            ...
                            )} type="date" class="input" id="check-out"></input>
                        </div>
                        ...
                        <div class="input input-group w-25">
                            ...
                            }} name="" id="room" class="options">
                                {rooms.map((value, index) => {
                                    return (<option value={index} key={value.id}>{value.roomType}</option>)
                                })}
                            </select>
                        </div>
                        <div class="input input-group w-25">
                            <label for="nor" class="input-label">Price</label>
                            <div className='options price-show'>{roomPrice}</div>
                        </div>


                    </form>
                    <button onClick={handleBooking} class="booking-btn">Booking</button>
